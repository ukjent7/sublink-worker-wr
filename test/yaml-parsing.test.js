...
const testCases = [
  ...
  - name: ANYTLS-main
    ...
    },
    {
        ...
        description: '粘贴配置中的 proxy-groups 被忽略，只提取节点填入出站',
        input: `proxies:
  - name: Valid-SS
    type: ss
    server: example.com
...
describe('YAML Parsing Tests', () => {
    testCases.forEach((testCase) => {
        it(testCase.name, async () => {
            const builder = new BaseConfigBuilder(testCase.input);
            const { subscriptions, singles } = await builder.parseInputsBySource();
            const items = [...singles, ...subscriptions.flat()];
